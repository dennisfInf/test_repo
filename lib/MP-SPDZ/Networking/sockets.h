    ...
    else if (j < 0)
    {
      if (errno == EAGAIN or errno == EINTR)
      {
        if (++fail > 25)
        {
          ...
          gethostname(err, 1000);
          strcat(err, " : ");
          strcat(err, "Unavailable too many times");
          throw runtime_error(string() + err + " : " + strerror(old_errno));
        }
        else
        {
...
inline ssize_t check_non_blocking_result(ssize_t res)
{
  if (res < 0)
  {
    if (errno != EWOULDBLOCK)
    {
      ...
      gethostname(err, 1000);
      strcat(err, " : ");
      strcat(err, "Non-blocking receiving error");
      throw runtime_error(string() + err + " : " + strerror(old_errno));
    }
    return 0;
  }
...
inline ssize_t receive_all_or_nothing(int socket, octet *msg, ssize_t len)
{
  ...
  if (res == len)
  {
    if (recv(socket, msg, len, 0) != len)
    {
      ...
      gethostname(err, 1000);
      strcat(err, " : ");
      strcat(err, "All or nothing receiving error");
      throw runtime_error(string() + err + " : " + strerror(old_errno));
    }
    return len;
  }
