...
namespace emp
{
    ...
    DORAM::DORAM(uint log_address_space_size, rep_array_unsliced<y_type> *ys_no_dummy_room, uint num_levels, uint log_amp_factor)
    ...
    void DORAM::logger_write(string msg)
    {
        if (party == 1)
        {
    ...
    uint DORAM::get_num_alive_levels()
    {
        uint cnt = 0;
    ...
    rep_array_unsliced<block> DORAM::generate_prf_key(uint level_num)
    {
    ...
    uint DORAM::get_log_col_len(uint level_num, uint _state)
    {
        assert(level_num < num_levels &&
               (_state == UINT_MAX ||
    ...
    uint DORAM::get_num_dummies(uint level_num) // B^i * stupid_level_size (not including stash size )
    {
    ...
    uint DORAM::num_elements_at(uint level_num, uint state_override) //*this depends on the state vector
    {
        assert(level_num < num_levels);
    ...
    uint DORAM::total_num_els_and_dummies(uint level_num, uint state_override)
    {
    ...
    rep_array_unsliced<y_type> DORAM::read_and_write(rep_array_unsliced<x_type> qry_x, rep_array_unsliced<y_type> qry_y,
    ...
    void DORAM::rebuild()
    {
        ...
        assert((rebuild_to == num_levels - 1) || tot_num_to_extract == num_elements_at(rebuild_to, base_b_state_vec[rebuild_to] + 1));
        ...
        if (rebuild_to == num_levels - 1) //*it is different to build to largest level
        {
            ...
            for (uint i = 0; i < num_levels; i++)
            {
                if (i == num_levels - 1 && !had_initial_bottom_level && base_b_state_vec[i] == 0)
                    break;
                delete_ohtable(i);
            }
    ...
    void DORAM::cleanse_bottom_level(rep_array_unsliced<x_type> extracted_list_xs,
    ...
    void DORAM::relabel_dummies(rep_array_unsliced<x_type> extracted_list_xs, uint log_N)
    {
        uint num_extracted = extracted_list_xs.length_Ts();
    ...
    void DORAM::insert_stash(uint level_num)
    {
    ...
    void DORAM::extract_alibi_bits(rep_array_unsliced<y_type> y_accum, rep_array_unsliced<int> alibi_mask)
    {
        for (uint i = 0; i < num_levels; i++)
        {
