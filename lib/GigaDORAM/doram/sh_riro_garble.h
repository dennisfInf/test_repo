...
namespace emp
{
    ...
    namespace sh_riro
    {
        ...
        static double time_before_compute;
        static double time_compute;
        static double time_after_compute;
        ...
        public:
            ...
            void adjust_recv_counter(int cnt)
            {
                recv_counter = cnt;
            }
        };
        ...
        class BitCircuit
        {
            ...
            HalfGateEva<RecvBufferIO<RepNetIO>> *half_gate_eva;
            SendBufferIO<RepNetIO> *sb_io;
            HalfGateGen<SendBufferIO<RepNetIO>> *next_gen;
            AbandonIO *g2_abandon_io;
            HalfGateGen<AbandonIO> *prev_gen;

        public:
            BitCircuit(BristolFashion_array &circuit_file)
                : circuit_file(circuit_file)
            {
                std::cout << "bit circ" << std::endl;
                if (party == 1)
                {
                    // todo: make this global
                    std::cout << "num and gate" << 2 * circuit_file.num_and_gate << std::endl;
                    rb_io = new RecvBufferIO<RepNetIO>(prev_io, 2 * circuit_file.num_and_gate);
                    rb_io->actually_receive(3);
                    half_gate_eva = new HalfGateEva<RecvBufferIO<RepNetIO>>(rb_io);
                }
                else if (party == 2)
                {
                    // G2
                    g2_abandon_io = new AbandonIO();
                    prev_gen = new HalfGateGen<AbandonIO>(g2_abandon_io);
                }
                else if (party == 3)
                {
                    // G1
                    sb_io = new SendBufferIO<RepNetIO>(next_io, 2 * circuit_file.num_and_gate);
                    next_gen = new HalfGateGen<SendBufferIO<RepNetIO>>(sb_io);
                    sb_io->flush();
                }
                std::cout << "bit circ finish" << std::endl;
            }
            ...
            void compute(rep_array_unsliced<block> &in, rep_array_unsliced<block> &out)
            {
                ...
                {
                    // todo: make this global
                    half_gate_eva->io->actually_receive();
                    circuit_file.compute_garbled(out_fixed.data(), in_fixed.data(), half_gate_eva);
                    std::cout << "finished computing" << std::endl;
                }
                else if (party == 2)
                {
                    // G2
                    circuit_file.compute_garbled(out_fixed.data(), in_fixed.data(), prev_gen);
                    std::cout << "finished computing" << std::endl;
                    prev_gen->io->flush();
                }
                else if (party == 3)
                {
                    circuit_file.compute_garbled(out_fixed.data(), in_fixed.data(), next_gen);
                    std::cout << "finished computing" << std::endl;
                    next_gen->io->flush();
