...
namespace emp
{
    static double time_total_network = 0;
    class RepSubChannel
    {
    public:
        int sock;
    ...
    public:
        ...
        int client_connect(const char *address, int port)
        {
            ...
            if (!quiet)
                cout << "trying to determine inet_addr" << endl;

            dest.sin_addr.s_addr = inet_addr(address);
            dest.sin_port = htons(port);

            if (!quiet)
                cout << "entering connect loop on " << address << ":" << port << endl;
            ...
            if (!quiet)
                cout << "connected as client on " << port << endl;
