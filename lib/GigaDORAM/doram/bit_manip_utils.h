...
template <typename T>
constexpr inline T get_all_ones_of_type() // ? the constexpr is highly questionable
{
    T all_ones;
...
template <typename T>
constexpr inline T get_all_zeros_of_type()
{
    T all_zeros;
...
inline Big _get_last_typesize_nuker() // was constexpr
{
...
template <typename q_type, typename x_type>
constexpr inline q_type get_lowest_x_type_many_bits_are_ones()
{
    q_type ones_arr;
...
template <typename T>
uint inline __cht_for_oht_hash(const T *el, bool col, uint num_bits)
{
...
template <typename T>
uint inline get_last_uint_bits(const T &el)
{
...
template <typename T>
T get_one_hot_mask(uint set_bit_index)
{
...
template <typename T>
T get_all_zero_except_nth_from_highest(uint n)
{
    assert(n < (8 * sizeof(T)));
...
template <typename T>
T get_all_ones_rightshifted_by(uint shift)
{
