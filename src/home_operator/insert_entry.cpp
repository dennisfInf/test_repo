...
namespace bookkeeping
{
  ...
  BenchmarkInsertEntry HomeOperatorMal::insert_entry(int &runs, int &batch_size)
  {
    ...
    for (int i = 0; i < runs; i++)
    {
      ...
      for (int j = 0; j < batch_size; j++)
      {
        ...
        std::cout << "received dec share: " << std::endl;
        c1.print_coordinates();
        el_gamal::Ciphertext ciphertext_2 = el_gamal.receive_ciphertext(this->tegService);
        std::cout << "received dec share: " << std::endl;
        G1 c1_2 = grpc::deserialize_from_string<G1>(ciphertext_2.c1())[0];
        c1_2.print_coordinates();

        std::cout << "test value 1 " << std::endl;
        G1 test = entry.proof.ciphertext.c2 - (c1 + share.share);
        test.norm();
        test.print_coordinates();
        std::cout << "test 1 share " << std::endl;
        c1.print_coordinates();
        std::cout << "bootstrap share " << std::endl;
        share.share.print_coordinates();

        std::cout << "test value 2 " << std::endl;

        G1 test2 = entry.proof.ciphertext.c2 - c1 - share.share - c1_2;
        test2.norm();
        test2.print_coordinates();

        std::cout << "test value 3 " << std::endl;

        G1 test3 = entry.proof.ciphertext.c2 - c1_2 - share.share;
        test3.norm();
        test3.print_coordinates();
  ...
  BenchmarkInsertEntry HomeOperator::calc_decryption_shares(BilinearGroup::BN &period, BilinearGroup::BN &msg, int &runs, int &batch_size, tsps::Protocol *tsps)
  {
    bigint::init_thread();
    std::future<void> oram_ready_fut = BilinearGroup::pool.push([](int) {});
    SafeQueue<bool> mpc_ready_queue;
    ...
    for (int i = 0; i < runs; i++)
    {
      ...
      for (int j = 0; j < batch_size; j++)
      {
        ...
        if (this->verify_create_entry_proof(p_ce, h, tsps))
        {
          ...
          std::cout << "send dec share: from" << static_cast<int>(this->ho_index) << std::endl;
          decryption_share.share.print_coordinates();
          el_gamal.share_point(decryption_share.share);
          auto end = std::chrono::high_resolution_clock::now();
          std::chrono::duration<double> elapsed = end - start;
          insert_entry_time += elapsed.count();
        }
      ...
      if (this->ho_index == 2)
      {
        std::cout << "ho 2 inputs" << std::endl;
        for (auto &dec_sharex : dec_shares)
        {
          dec_sharex.share.norm();
          dec_sharex.share.print_coordinates();
        }
      }
      mpc_client.send_G1_points(dec_shares, batch_size);
  ...
  void HomeOperator::input_to_mpcs(std::chrono::_V2::system_clock::time_point &start_mpc1, bool bootstrap, double &mpc_time1, double &mpc_time2, std::future<void> &oram_ready_fut, SafeQueue<bool> &mpc_ready_queue, int &batch_size)
  {
