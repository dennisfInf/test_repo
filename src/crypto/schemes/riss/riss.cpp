...
namespace RISS
{
  ...
  RISSInteger Scheme::get_secret_ras_r_value()
  {
    ...
    futures.push_back(BilinearGroup::pool.push([this, &secret](int)
                                               { secret = BilinearGroup::BN::rand(this->l, 1); }));
  ...
  {
    BilinearGroup::BN res;
    BilinearGroup::BN::sub_without_mod(res, r_value, sub_proc::calculate_share(shares, &get_y_values_p(), BilinearGroup::BN::get_field_prime_number()));
    return res;
  }
  ...
  BilinearGroup::BN Scheme::calc_share_plus_r(BilinearGroup::BN &f_p_share, BilinearGroup::BN &riss_share)
  {
    ...
    BilinearGroup::BN::mul_without_mod(mul_res, riss_share, la_grange_coeffs_p[my_index - 1]);
    BilinearGroup::BN add_res;
    BilinearGroup::BN::add_without_mod(add_res, f_p_share, mul_res);
    return add_res;
  }

  std::vector<BilinearGroup::BN> Scheme::calculate_lagrange_coeffs()
  {
    BilinearGroup::BN mod = BilinearGroup::BN::get_field_prime_number();
  ...
  BilinearGroup::BN Scheme::open_value(std::vector<Share> &shares)
  {
    ...
    result %= BilinearGroup::BN::get_field_prime_number();
    return result;
  }
  ...
  std::vector<BilinearGroup::BN> test_riss(int n, int t)
  {
    ...
    for (int i = 0; i < n; i++)
    {
      players.push_back(Scheme(i + 1, t, n, 4, 8, BilinearGroup::BN::get_field_prime_number()));
    ...
    for (int i = 1; i <= t + 1; i++)
    {
      ...
      for (int j = 0; j < n; j++)
      {
        BilinearGroup::BN::add_without_mod(player_shares_sum, player_shares_sum, initial_shares[i - 1][j]);
        player_shares_sum %= mod;
      }
      final_shares.push_back(player_shares_sum);
      BilinearGroup::BN mul_res;
      BilinearGroup::BN::mul_without_mod(mul_res, player_shares_sum, la_grange_coeffs[i - 1]);
      mul_res %= mod;
      BilinearGroup::BN::add_without_mod(left, left, mul_res);
      left %= mod;
      BilinearGroup::BN::add_without_mod(right, right, riss_ints[i - 1].secret);
      right %= mod;
    }
    if (left == right)
    {
  ...
  void test_sub_protocol(uint8_t n, uint8_t t)
  {
    ...
    }
    {
      ...
      BilinearGroup::BN mod_p = BilinearGroup::BN::get_field_prime_number();
      BilinearGroup::BN left;
      BilinearGroup::BN right;
      for (int i = 1; i <= t + 1; i++)
      {
        ...
        BilinearGroup::BN mul_res;
        BilinearGroup::BN::mul_without_mod(mul_res, r_shares[i - 1].r_p, la_grange_coeffs_p[i - 1]);
        mul_res %= mod_p;
        BilinearGroup::BN::add_without_mod(left, left, mul_res);
        left %= mod_p;
        BilinearGroup::BN::add_without_mod(right, riss_ints[i - 1].secret, right);
        right %= mod_p;
      }
      std::cout << "left does not work always??" << std::endl;
      left.print();
      right.print();
      if (left == right)
      {
      ...
      for (int i = 0; i < n; i++)
      {
        BilinearGroup::BN val;
        BilinearGroup::BN::mul_without_mod(val, secrets[i], la_grange_coeffs_p[i]);
      ...
      BilinearGroup::BN secret_r;
      BilinearGroup::BN::add_without_mod(secret_r, secrets[n], right);
      secret_r %= BilinearGroup::BN::get_field_prime_number();
      if (fp_val_plus_r == secret_r)
      {
  ...
  void test_sub_protocol_mal(uint8_t n, uint8_t t)
  {
    std::string str1 = "249 213 225 206 39 184 58 153";
    std::stringstream ss(str1);
    std::vector<uint8_t> vec1;
