...
namespace Polynomial
{
  ...
  std::vector<BilinearGroup::BN> generate_polynomial(const uint8_t &num_coefficients)
  {
    ...
    {
      futures.push_back(
          BilinearGroup::pool.push([&coefficient = polynomial[i]](int)
                                   { coefficient = BilinearGroup::BN::rand(); }));
    }
    for (auto &f : futures)
    {
      f.wait();
  ...
  {
    return get_lagrange_coeff_start(x_coord, player_index, 1, n);
  }
  ...
  BilinearGroup::BN get_lagrange_coeff_start(const BilinearGroup::BN &x_coord, const uint8_t &player_index, const uint8_t start, const uint8_t &n)
  {
    ...
    for (int j = start; j < n + 1; j++)
    {
      ...
      denominator = denominator * (player_index_bn - j_bn);
    }
