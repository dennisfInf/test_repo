...
for id in {0..$(($n- 1))}; do
    docker run  --rm  --network host --entrypoint /bin/bash  --mount type=bind,source=$script_dir/../mp-spdz-files/HostFiles/local/hostnames_mpc2.txt,target=/usr/src/MP-SPDZ/hostsmpc.txt --mount type=bind,source=$script_dir/../benchmarks,target=/app/benchmarks mpspdz -c "sleep 2 && exec ./shamir-party.x $id oram_honest-$n-$5-$6-$t-1 -pn 19500 --ip-file-name hostsmpc.txt -N $n -B 5 --batch-size 100 > /app/benchmarks/mpspdz-$n-$id-$t-mpc2.log 2>&1" &
done
sleep 8
for id in {0..$(($n- 1))}; do
