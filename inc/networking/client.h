...
{
template <typename T> class Stub
{
public:
  Stub(std::shared_ptr<grpc::Channel> channel) : stub_(T::NewStub(channel)) {}
  template <typename U, typename V, typename Func> V send(U &request, uint32_t participant_index, Func func)
  {
    V response;
    grpc::Status status;
    uint8_t counter = 0;
...
public:
  ...
  Client(std::string address, int index)
      : index(index), channel(grpc::CreateChannel(address, grpc::InsecureChannelCredentials()))
  {
  }
  template <typename T> Stub<T> createStub() { return Stub<T>(channel); }
